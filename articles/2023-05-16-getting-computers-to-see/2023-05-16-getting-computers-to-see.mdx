...
**_An image is a set of number, with typically three colour channels: Red,
Green, Blue._**
...
<div style={{ display: 'flex', gap: '10px' }}>
    ...
    <div style={{ width: '60%' }}>
        <Carousel images={[
            { src: image1, alt: 'Description for image 1', title: 'Busy Background', text: 'Cluttered or textured background can make it difficult to determine the boundaries for each object.' },
            { src: image2, alt: 'Description for image 2', title: 'Yale Database of Faces', text: 'Variations in lighting can make it difficult to recognise the same person (recall: computers “see” pixel values).' },
            { src: image3, alt: 'Description for image 3', title:'Rick Scuteri-USA TODAY Sports', text:'Different scales can make it difficult to understand context from images.'},
            { src: image4, alt: 'Description for image 4', title:'Michelangelo (1475-1564)', text:'Occlusion caused by different viewpoints can make it difficult to recognise the same person.' },
        ]} />
    </div>
    ...
    <div style={{ width: '40%'}}>
        ...
        When an image is registered, there is an inherent loss of information, as the 3D world is projected onto a 2D plane.

        Even for us, collapsing the _spatial information we get from the physical world
        can make it difficult to discern what we are looking at through photos_.

        It can be difficult to recognise objects in 2D pictures due to the following
        _ill-posed_ properties....
    </div>
...
<figcaption style={{textAlign: 'center', fontSize:'small'}}>Point of view makes it impossible to identify the cans.</figcaption>
...
<figcaption style={{textAlign: 'center', fontSize:'small'}}>Impossible to describe original scene due to uncertainty from blurring.</figcaption>
...
<figcaption style={{textAlign: 'center', fontSize:'small'}}>Chairs take on many different forms.</figcaption>
...
<div style={{textAlign: 'center', fontStyle: 'bold'}}>
_While, at first glance, computer vision tasks may appear superficial, the underlying problem they are trying to address is quite challenging!_
</div>
...
<div style={{flex: 2, marginRight: '10px'}}>
In order for a computer vision model to overcome the inherent ill-posed problems that are embedded in the images we provide it, we must modify the inverse equation to solve for alternative, yet similar, well-posed problem instead.
</div>
